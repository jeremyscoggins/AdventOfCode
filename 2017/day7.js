...
for(var i = 0; i < inputArray.length; i++)
{
    ...
    var node = {
        name : m[1],
        weight : parseInt(m[2]),
        children : m[4] != undefined ? m[4].split(", ") : []
    };
    nodes.push(node);
}
...
do {
    ...
    for(var i = 0; i < nodes.length; i++)
    {
        ...
        {
            if(current.children.indexOf(node.name) != -1)
            {
                foundParent = true;
                node = current;
                break;
...
console.log("root node is: " + node.name);
...
function walk(node)
{
    ...
    for(var i = 0; i < nodes.length; i++)
    {
        ...
        if(current.name != node.name)
        {
            if(node.children.indexOf(current.name) != -1)
            {
                totalWeight += walk(current);
                node.childNodes.push(current);
            }
        }
    }
