...
var pattern = /([a-z]+) ([a-z]+) (-?\d+) if ([a-z]+) ([<>=!]+) (-?\d+)/

var inputArray = input.split("\n");

var registers = {};

var maxEver = -1000000000;
for(var i = 0; i < inputArray.length; i++)
{
    ...
    var operation = m[2];
    var amount = m[3];
    var compareRegister = m[4];
    var comparison = m[5];
    var compareAmount = m[6];
    ...
    if(!registers.hasOwnProperty(compareRegister))
    {
        registers[compareRegister] = 0;
    }

    if(eval(registers[compareRegister] + " " + comparison + " " + compareAmount))
    {
        if(operation == "inc")
        {
            registers[register] += parseInt(amount);
        }
        else
        {
            registers[register] -= parseInt(amount);
    ...
    maxEver = registers[register] > maxEver ? registers[register] : maxEver;
...
{
    max = registers[keys[i]] > max ? registers[keys[i]] : max;
}

console.log("max register value is : " + max);
console.log("max ever register value is : " + maxEver);