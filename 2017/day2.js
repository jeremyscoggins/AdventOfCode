...
for(var i = 0; i < input.length; i++)
{
  ...
  var min = 10000;
  var max = 0;
  for(var item in grid[i])
  {
    var number = parseInt(grid[i][item]);
    //console.log(number);
    if(number > max)
    {
      max = number;
    ...
    for(var item2 in grid[i])
    {
      if(item2 != item)
      {
        var number2 = parseInt(grid[i][item2]);
        if(number % number2 == 0)
        {
          console.log(number + ' ' + number2);
          checksum2 += number / number2;
        }
      }
